...
export default {
  ...
  methods: {    
    ...
    getList() {
      this.loading = true
      // 获取列表
      this.$api.getSupervisionTempList(this.listQuery).then(response => {     
    ...
    handleUpdate(data) {   
      ...
      this.$nextTick(_ => {
        this.$refs.logsForm.resetFields()
        this.logsForm.tempId = data.tempId
    ...
    validateCheckTemplateCode(isChange) {
      if (this.logsForm.batchCode) {
        if (!isChange) return
        this.$api.checkSuperviTemplateCode({
          batchCode: this.logsForm.batchCode,
          templateId: this.logsForm.tempId
        }).then(response => {
          if (response.data[0].isExist) {
            this.$message.warning(response.resultMsg)
            this.$refs.logsForm.validateField('batchCode')
            return false
          }
        })
      }
    ...
    create() {
      ...
      delete this.logsForm.tempId  
      _this.$refs.logsForm.validate(valid => {
        if (valid) {
          _this.$api.addSupervisionTemp(_this.logsForm).then(response => {
            if (response.errorCode === '1') {
              _this.getList()
    ...
    update(data) {  
      ...
      _this.$refs.logsForm.validate(valid => {
        if (valid) {
          _this.$api.updateSupervisionTemp(_this.logsForm).then(response => {
            if (response.errorCode === '1') {
              _this.getList()
