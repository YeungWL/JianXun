  ...
  <div class="page-content-body">
    <div class="page-header clearfix">
        ...
        <el-row :gutter="24">
          <el-col :span="10">
            <el-form-item label="会议名称：" label-width="90px">
              <el-input v-model="listMeetingQuery.meetingTitle" placeholder="会议名称"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="14">
            <el-form-item label="会议地址" label-width="90px">
              <el-input v-model="listMeetingQuery.meetingPlace" placeholder="会议地址"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <el-form ref="form" v-model="listMeetingQuery" size="mini">
        <el-row :gutter="24">
          ...
          <el-col :span="5">
            <el-form-item label="通知状态：" prop="noticeType" label-width="90px">
          ...
          <el-col :span="5">
            <el-form-item label="取消状态：" prop="stateType" label-width="90px">
              <el-select v-model="listQuery.stateType" placeholder="请选择" style="width: 100%;">
                <el-option v-for="item in statusQuery" :key="item.value" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="会议时间：" prop="EnterTime" label-width="90px">
              <el-date-picker align="right" unlink-panels v-model="meetingTimeRange" type="daterange" start-placeholder="开始日期"
                end-placeholder="结束日期" range-separator="-" value-format="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm"
                :picker-options="pickerOptions2" :default-time="['00:00:00', '23:59:59']" @change="timeRangeChange"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <div class="page-main customTable">
      <el-table ref="multipleTable" style="width: 100%" v-loading="loading" element-loading-text="拼命加载中" :data="meetingListCurrent">
        ...
        <el-table-column prop="meetingBeginTime" label="会议时间" min-width="80" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="attendType" label="参加人" min-width="80" show-overflow-tooltip></el-table-column>
          ...
          <el-table-column label="操作" min-width="100">
            <template slot-scope="scope">
              ...
              <span class="edit" @click="editDetails(scope.row)" v-show="scope.row.isMyCreate === 'Y'"  >修改</span>
              <span v-show="scope.row.isMyCreate === 'Y'">
              <el-switch v-model="scope.row.stateType === 'Y'" active-color="#13ce66" inactive-color="#ff4949"
                @change="switchChange(scope.row)" ></el-switch>
              </span>
              <span class="edit" @click="documentPublicHandle(scope.row)" v-show="scope.row.isMyCreate=== 'Y'" >{{ scope.row.putaway=== "Y"? '下架' : '上架' }}</span>
            </template>
          </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog title="导入情况" :visible.sync="importBatchResDialogVisible" width="360px" size="tiny" class="my-dialog batchAddWorkerDialog"
      ...
      <div class="dialog-content">
        <div style="margin-left: 80px;">
          <span>成功 :</span>
          <span style="color:#2AB452;">{{addWorkerSuccessCount}} 条</span>
        </div>
        <div style="margin-left: 80px;">
          <span>失败 :</span>
          <span style="color:#FF0000;">{{addWorkerFailCount}} 条</span>
        </div>
  ...
  methods: {
    ...
    currentMeetingList() {
      ...
      this.$api.meetingList(this.listMeetingQuery).then(res => {
        if (res.errorCode === "1") {
          this.meetingListCurrent = res.data.meetingList;
          console.log("Whybee " + this.meetingListCurrent);
        } else {
