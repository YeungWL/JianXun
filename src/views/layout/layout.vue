    ...
    <header class="header">
      ...
      <div class="nav">
        <!--<el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal"
                 @select="handleSelect">
          <el-menu-item :index="item.menuId"   class="index" v-for="(item,index) in primaryRouteMenuList" :key="index" v-if="item.status">
            <i class="iconfont icon-xiangmuguanli" 
            :class="{
            'iconfont icon-xiangmuguanli':item.menuId=='12',
            'iconfont icon-zuzhijigouguanli':item.menuId=='14',
            'iconfont icon-shigongrizhi':item.menuId=='29',
            'iconfont icon-wenjianguanli':item.menuId=='13',
            'iconfont icon-xitongshezhi':item.menuId=='1'
            }"
...
export default {
  ...
  mounted(){
    ...
    //   console.log("primaryRouteMenuList:"+this.rootLists);
  },  
  methods: {
    // 获取用户权限
    getMyMenuList() {
    ...
    //     if(res.errorCode === '1'){
    //       localStorage.setItem('Roots',JSON.stringify(res.data));
    //       // this.$store._mutations.Roots(res.data);
    //       console.log(localStorage.setItem('Roots',JSON.stringify(res.data)))
    //     }
    //   })
    // },        
